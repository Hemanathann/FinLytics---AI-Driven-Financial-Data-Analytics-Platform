...
import { OnboardingChecklist, SpendingAlerts, NetWorthTimeline } from "./dashboard-widgets-new";
...
// No per-account currency prefix needed anymore
...
export function DashboardClient({ accounts, transactions, savingsGoals, subscriptionData, netWorthData, budgetVsActual, budgetData }) {
  ...
  const handleExportCSV = async () => {
    ...
    try {
      ...
      if (result?.csv) {
        ...
        // GA4: user exported their transaction data
        trackCSVExported();
      }
    } catch (err) {
  ...
  const sym      = "€";   // currency symbol for input prefixes
  const currency = "EUR"; // currency code shown on account badges
  ...
  const detectedSubs = KNOWN_SUBS.map(sub => {
    const matches = transactions.filter(t => (t.description||"").toLowerCase().includes(sub.key));
    if (matches.length === 0) return null;
    const latest = matches.sort((a,b)=>new Date(b.date)-new Date(a.date))[0];
    return { ...sub, amount: Number(latest.amount), lastDate: latest.date, occurrences: matches.length };
  }).filter(Boolean);
  ...
  const handleBudgetSave = async () => {
    ...
    // GA4: user set their monthly budget
    trackBudgetSet({ amount: amt });
    setBudgetEdit(false); toast.success("Budget updated");
  };
  ...
  const handleGoalCreate = async () => {
    ...
    await createGoalFn(goalData); setGoalForm(false);
    // GA4: user created a savings goal
    ...
    toast.success("Goal created!"); window.location.reload();
  };
  ...
  const handleAddInsurance = () => {
    ...
    setManualIns(false); toast.success("Insurance added");
  };
  ...
  return (
    <div className="pb-20 max-w-5xl mx-auto space-y-5">
        ...
        {budgetEdit ? (
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              <input value={budgetVal} onChange={e=>setBudgetVal(e.target.value)} type="number" placeholder="Monthly budget"
      ...
      <div className="flex gap-1 p-1 bg-muted rounded-2xl overflow-x-auto">
        {tabs.map(t=>(
          <button key={t.id} onClick={()=>setActiveTab(t.id)}
            className={cn("flex-1 text-xs sm:text-sm py-2 px-2 sm:px-3 rounded-xl font-medium transition-all whitespace-nowrap",
      ...
      {/* ════════════════════════════════════════════════════════
          TAB: OVERVIEW — Accounts + Goals summary
      ════════════════════════════════════════════════════════ */}
      {activeTab==="overview" && (
        <div className="space-y-6">
          {/* Onboarding checklist — only shown to new users, dismissable */}
          <OnboardingChecklist
            hasAccounts={accounts.length > 0}
            hasTransactions={transactions.length > 0}
            hasBudget={!!(budgetData?.amount)}
            hasGoals={!!(savingsGoals?.length > 0)}
          />

          {/* Spending alerts — category budget warnings */}
          ...
          {/* Empty state — no transactions yet: show demo data CTA */}
          ...
          {/* Accounts */}
          <div>
            <SectionHead title="Accounts" subtitle={`${accounts.length} account${accounts.length!==1?"s":""} · ${fmt(totalBalance)} total`} action={
              <CreateAccountDrawer>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {accounts.map((acc, i)=>{
                ...
                return (
                  <div key={acc.id} className={cn("group relative p-5 rounded-2xl border bg-card hover:shadow-md transition-all overflow-hidden",
                    ...
                    <div className="relative">
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex items-center gap-2.5">
                          ...
                          <span className="text-[10px] bg-muted px-1.5 py-0.5 rounded-full font-mono">{getAccountCurrency(acc.name) || currency}</span>
                      ...
                      <div className="flex items-center gap-2 mt-4 pt-4 border-t">
                        ...
                        <div className="flex gap-1">
                          ...
                          {!acc.isDefault && (
                            <button onClick={()=>handleDeleteAccount(acc)} disabled={deletingId===acc.id}
          ...
          {/* Spending charts */}
          <div>
            <SectionHead title="Spending Overview" subtitle="Last 30 days · charts update live" action={
          ...
          {/* Net worth timeline — most motivating personal finance chart */}
          {netWorthData?.chartData?.length >= 2 && (
            <div>
              <SectionHead title="Net Worth Timeline" subtitle="Month by month · all accounts" action={
                <Link href="/analytics" className="text-xs text-blue-600 font-semibold flex items-center gap-1">
                  Full analytics<ChevronRight className="h-3.5 w-3.5"/>
                </Link>
              }/>
          ...
          {(savingsGoals||[]).length>0 && (
            <div>
              <SectionHead title="Savings Goals" subtitle="Your financial targets" action={
      ...
      {activeTab==="transactions" && (
        <div className="space-y-4">
          ...
          <div className="rounded-2xl border bg-card overflow-hidden divide-y">
            {recentTxns.length===0 ? (
              <EmptyState icon={BarChart2} title="No transactions yet" sub="Add your first transaction to get started"
      ...
      {activeTab==="subscriptions" && (
        <div className="space-y-6">
          ...
          <div>
            <SectionHead title="Streaming & Digital Subscriptions" subtitle="Auto-detected from your transactions" action={
            ...
            {manualSub && (
              <div className="p-4 rounded-2xl border bg-muted/30 mb-4 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <input value={newSub.name} onChange={e=>setNewSub(s=>({...s,name:e.target.value}))} placeholder="Service name"
                  ...
                  <input value={newSub.amount} onChange={e=>setNewSub(s=>({...s,amount:e.target.value}))} type="number" placeholder={`Amount (${sym})`}
                    className="bg-background rounded-xl px-3 py-2.5 text-sm outline-none border focus:ring-2 focus:ring-blue-500/30"/>
                  <select value={newSub.frequency} onChange={e=>setNewSub(s=>({...s,frequency:e.target.value}))}
                    className="bg-background rounded-xl px-3 py-2.5 text-sm outline-none border focus:ring-2 focus:ring-blue-500/30">
      ...
      {activeTab==="insurance" && (
        <div className="space-y-5">
          <SectionHead title="Insurance Tracker" subtitle="Track all your policies in one place" action={
          ...
          {manualIns && (
            <div className="p-5 rounded-2xl border bg-muted/30 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                <select value={newIns.type} onChange={e=>setNewIns(s=>({...s,type:e.target.value}))}
                  className="col-span-2 bg-background rounded-xl px-3 py-2.5 text-sm outline-none border focus:ring-2 focus:ring-blue-500/30">
                  {["Life","Health","Vehicle","Home","Travel","Term Life","Critical Illness","Pet","Business"].map(t=><option key={t}>{t}</option>)}
                </select>
                <input value={newIns.name} onChange={e=>setNewIns(s=>({...s,name:e.target.value}))} placeholder="Policy name"
                  className="bg-background rounded-xl px-3 py-2.5 text-sm outline-none border focus:ring-2 focus:ring-blue-500/30"/>
                <input value={newIns.provider} onChange={e=>setNewIns(s=>({...s,provider:e.target.value}))} placeholder="Provider (e.g. LIC, Aviva)"
                  className="bg-background rounded-xl px-3 py-2.5 text-sm outline-none border focus:ring-2 focus:ring-blue-500/30"/>
                <input value={newIns.premium} onChange={e=>setNewIns(s=>({...s,premium:e.target.value}))} type="number" placeholder={`Premium (${sym})`}
                  className="bg-background rounded-xl px-3 py-2.5 text-sm outline-none border focus:ring-2 focus:ring-blue-500/30"/>
                <select value={newIns.frequency} onChange={e=>setNewIns(s=>({...s,frequency:e.target.value}))}
                  className="bg-background rounded-xl px-3 py-2.5 text-sm outline-none border focus:ring-2 focus:ring-blue-500/30">
                  <option value="monthly">Monthly</option>
                  <option value="quarterly">Quarterly</option>
                  <option value="yearly">Yearly</option>
                ...
                <div>
                  ...
                  <input value={newIns.startDate} onChange={e=>setNewIns(s=>({...s,startDate:e.target.value}))} type="date"
                ...
                <div>
                  ...
                  <input value={newIns.endDate} onChange={e=>setNewIns(s=>({...s,endDate:e.target.value}))} type="date"
                    className="w-full bg-background rounded-xl px-3 py-2.5 text-sm outline-none border focus:ring-2 focus:ring-blue-500/30"/>
                </div>
              </div>
              <div className="flex gap-2">
                ...
                return (
                  <div key={ins.id} className={cn("p-5 rounded-2xl border bg-card",isExpiringSoon&&"border-amber-400 dark:border-amber-600")}>
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <button onClick={()=>setInsurances(prev=>prev.filter(i=>i.id!==ins.id))}
          ...
          {Object.values(detectedInsurances).length===0 && insurances.length===0 && (
            <EmptyState icon={Shield} title="No insurance policies tracked" sub="Add your life, health, vehicle or home insurance to track premiums and renewal dates"
      ...
      {activeTab==="goals" && (
        <div className="space-y-4">
          <SectionHead title="Savings Goals" subtitle={`${(savingsGoals||[]).filter(g=>g.completed).length} of ${(savingsGoals||[]).length} completed`} action={
          ...
          {goalForm && (
            <div className="p-5 rounded-2xl border bg-muted/30 space-y-3">
              <div className="flex flex-wrap gap-2">
                {GOAL_EMOJIS.map(e=>(
                  <button key={e} onClick={()=>setGoalData(d=>({...d,emoji:e}))}
              ...
              <input value={goalData.name} onChange={e=>setGoalData(d=>({...d,name:e.target.value}))} placeholder="Goal name (e.g. Holiday in Japan)"
                className="w-full bg-background rounded-xl px-3 py-2.5 text-sm outline-none border focus:ring-2 focus:ring-blue-500/30"/>
              <div className="grid grid-cols-2 gap-2">
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-muted-foreground">{sym}</span>
                  <input value={goalData.targetAmount} onChange={e=>setGoalData(d=>({...d,targetAmount:e.target.value}))} type="number" placeholder="Target amount"
                ...
                <input value={goalData.targetDate} onChange={e=>setGoalData(d=>({...d,targetDate:e.target.value}))} type="date" min={new Date().toISOString().split("T")[0]}
                  className="bg-background rounded-xl px-3 py-2.5 text-sm outline-none border focus:ring-2 focus:ring-blue-500/30"/>
              </div>
              <div className="flex gap-2">
          ...
          {(savingsGoals||[]).length===0&&!goalForm ? (
            <EmptyState icon={Target} title="No savings goals yet" sub="Set targets and track your progress towards financial milestones"
                ...
                return (
                  <div key={goal.id} className={cn("p-5 rounded-2xl border bg-card",goal.completed&&"opacity-80")}>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-2 flex-wrap">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button onClick={()=>{ deleteGoalFn(goal.id); window.location.reload(); }}
                              className="text-muted-foreground hover:text-red-500 transition-colors">
                    ...
                    <div className="relative h-3 bg-muted rounded-full overflow-hidden mb-2">
                      <div className="h-full rounded-full transition-all duration-700 bg-gradient-to-r from-blue-500 via-indigo-500 to-purple-500"
                        style={{width:`${pct}%`}}/>
                      {pct>10 && (
